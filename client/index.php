...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['accion']) && $_POST['accion'] === 'eliminar') {
        if ($id) {
            $stmt = mysqli_prepare($conn, "DELETE FROM clientes WHERE id_cliente = ?");
    ...
    // ── Guardar (insertar o actualizar) ───────────────────────────────────────
        ...
        if (!$nombre || !$apellido || !$email || !$telefono) {
            ...
            // Verificar duplicado de email (excluyendo al propio si es edición)
            $stmt = mysqli_prepare($conn, "SELECT id_cliente FROM clientes WHERE email=? AND id_cliente!=?");
            ...
            if (mysqli_stmt_num_rows($stmt) > 0) {
                ...
                $stmt = mysqli_prepare($conn, "UPDATE clientes SET nombre=?,apellido=?,email=?,telefono=? WHERE id_cliente=?");
                mysqli_stmt_bind_param($stmt, 'ssssi', $nombre, $apellido, $email, $telefono, $id);
                mysqli_stmt_execute($stmt);
                $mensaje = '✓ Cliente actualizado correctamente';
            } else {
                // Nuevo cliente: estatus inactivo por defecto (se activará al registrar pago)
                $stmt = mysqli_prepare($conn, "INSERT INTO clientes(nombre,apellido,email,telefono,estatus,fecha_registro) VALUES(?,?,?,?,'inactivo',NOW())");
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                        ...
                        <div class="flex gap-3 w-full md:w-auto">
                            <form method="GET" class="flex gap-2 flex-1 md:flex-initial">
                                ...
                                <?php if ($busqueda): ?>
                                    <a href="index.php" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition">
